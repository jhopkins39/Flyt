...
class _CardsSectionState extends State<CardsSectionAlignment>
  ...
  _rightSwipe() {
     _getProfileData();
    String matchUID = cards[0].user.uid;

    if (user.matches == null) {
      user.matches = List();
    }

    if (!user.matches.contains(matchUID)) {
      user.matches.add(matchUID);
    }
    if (cards[0].user.matches != null && cards[0].user.matches.contains(matchUID)) {
      _match();
    }
    String uid = user.uid;
    _saveData(uid);
  ...
  _saveData(String uid) async {
    await Provider.of(context).db.collection('users').document(uid).setData(user.toJson());
  }
